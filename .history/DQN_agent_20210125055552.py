...
class DQN:
    ...
    def build_modelGPU(self, input_shape=(4, 100, 100,), action_space=6, dueling=True):
        ...
        X = Conv2D(64, 8, strides=(4), activation="relu",  padding="valid",
               kernel_initializer = 'random_uniform', bias_initializer = 'random_uniform',data_format='channels_first')(X)
        X = Conv2D(64, 4, strides=(2), activation="relu",  padding="valid",
               kernel_initializer = 'random_uniform', bias_initializer = 'random_uniform', data_format='channels_first')(X)
        X = Flatten()(X)
        X = Dense(self.network_size,  activation="relu",
                 kernel_initializer = 'random_uniform', bias_initializer = 'random_uniform')(X)
        state_value = Dense(1,kernel_initializer='sigmoid', bias_initializer = 'random_uniform')(X)
        state_value = Lambda(lambda s: K.expand_dims(
            s[:, 0], -1), output_shape=(self.action_space,))(state_value)
        action_advantage = Dense(
            self.action_space , kernel_initializer = 'random_uniform', bias_initializer = 'random_uniform')(X)
        action_advantage = Lambda(lambda a: a[:, :] - K.mean(
            a[:, :], keepdims=True), output_shape=(self.action_space,))(action_advantage)
        ...
        rms = optimizer=RMSprop(lr=self.learning_rate, rho=0.95, epsilon=self.epsilon)
        model.compile(loss = {'o_P': logloss, 'o_V': sumofsquares}, loss_weights = {'o_P': 1., 'o_V' : 0.5}, optimizer = rms, metrics=["accuracy"])
        model.summary()
        return model
