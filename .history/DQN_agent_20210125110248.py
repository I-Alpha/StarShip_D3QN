...
class DQN:
    ...
    def build_modelGPU(self, input_shape=(4, 100, 100,), action_space=6, dueling=True):
        ...
        X = Dense(512, activation="relu", kernel_initializer='')(X)
        ...
        if dueling:
            state_value = Dense(1, kernel_initializer='he_uniform' ,activation = 'sigmoid')(X)
            state_value = Lambda(lambda s: K.expand_dims(s[:, 0], -1), output_shape=(action_space,))(state_value)

            action_advantage = Dense(action_space, kernel_initializer='he_uniform',activation = 'relu')(X)
