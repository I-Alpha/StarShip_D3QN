...
class DQN:
    ...
    def build_modelGPU(self, input_shape=(4, 100, 100,), action_space=6, dueling=True):
        ...
        X = Conv2D(16, 5, strides=(3, 3),padding="valid", input_shape=input_shape, activation="relu", data_format="channels_first")(X)
        X = Conv2D(32, 4, strides=(2, 2),padding="valid", activation="relu", data_format="channels_first")(X)
        X = Conv2D(32, 3, strides=(1, 1),padding="valid", activation="relu", data_format="channels_first")(X)
        X = Flatten()(X)
        ...
        if dueling:
            state_value = Dense(1, kernel_initializer='random_uniform' ,activation = 'sigmoid')(X)
        ...
        model.compile(loss="mean_squared_error", optimizer=Adam(lr=0.00025, rho=0.95, epsilon=0.01), metrics=["accuracy"])
        model.summary()
        return model
