...
class DQN:
    ...
    def build_modelGPU(self, input_shape=(4, 100, 100,), action_space=6, dueling=True):
        ...
        X = Conv2D(32, 5, strides= [1, stride, stride, 1),padding="valid", kernel_initializer=truncatedn_init, bias_initializer=const_init, activation="relu" , data_format="channels_first")(X)
        X = Conv2D(64, 4, strides=(2, 2),padding="valid", kernel_initializer=truncatedn_init, bias_initializer=const_init, activation="relu", data_format="channels_first")(X)
        X = Conv2D(64, 3, strides=(1, 1),padding="valid",kernel_initializer=truncatedn_init, bias_initializer=const_init,  activation="relu", data_format="channels_first")(X)
        X = Flatten()(X)
        ...
        if dueling:
            state_value = Dense(1, kernel_initializer='random_uniform' ,activation = 'softmax')(X)
