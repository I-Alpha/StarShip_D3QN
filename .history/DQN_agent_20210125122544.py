...
class DQN:
    ...
    def saveModel(self, score="n.a"):
        print(self.model.name+str(int(score))+"-" +
    ...
    def build_modelGPU(self, input_shape=(1,4, 100, 100,1), action_space=6, dueling=True):
        ...
        X = Conv2D(32, 5, strides=(4),padding="same", kernel_initializer=truncatedn_init, bias_initializer=const_init, activation="relu" , data_format="channels_first")(X)
        X = Conv2D(64, 4, strides=(2),padding="same", kernel_initializer=truncatedn_init, bias_initializer=const_init, activation="relu", data_format="channels_first")(X)
        X = Conv2D(64, 3, strides=(1),padding="same",kernel_initializer=truncatedn_init, bias_initializer=const_init,  activation="relu", data_format="channels_first")(X)
        X = Flatten()(X)
        # 'Dense' is the basic form of a neural network layer
        # Input Layer of state size(4) and Hidden Layer with 512 nodes
        X = Dense(512, activation="relu", kernel_initializer=truncatedn_init, bias_initializer=const_init)(X)
        ...
        if dueling:
            state_value = Dense(1,kernel_initializer=truncatedn_init, bias_initializer=const_init, activation = 'relu')(X)
            state_value = Lambda(lambda s: K.expand_dims(s[:, 0], -1), output_shape=(action_space,))(state_value)

            action_advantage = Dense(action_space,kernel_initializer=truncatedn_init, bias_initializer=const_init, activation = 'relu')(X)
