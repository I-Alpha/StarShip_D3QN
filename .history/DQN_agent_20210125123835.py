...
class DQN:
    ...
    def build_modelGPU(self, input_shape=(4, 100, 100), action_space=6, dueling=True):
        ...
        X = Conv2D(32, 5, strides=(4),padding="same", kernel_initializer=truncatedn_init, bias_initializer=const_init, use_bias= activation="relu" , data_format="channels_first")(X)
        ...
        X = Dense(128, activation="relu", kernel_initializer=truncatedn_init, bias_initializer=const_init)(X)        
        ...
        if dueling:
            state_value = Dense(1,kernel_initializer=truncatedn_init, bias_initializer=const_init, activation = 'softmax')(X)
            state_value = Lambda(lambda s: K.expand_dims(s[:, 0], -1), output_shape=(action_space,))(state_value)

            action_advantage = Dense(action_space,kernel_initializer=truncatedn_init, bias_initializer=const_init, activation = 'linear')(X)
