...
class DQN:
    ...
    def build_modelGPU(self, input_shape=(None,4, 100, 100), action_space=6, dueling=True):
        ...
        X = Dense(128, activation="relu", kernel_initializer=truncatedn_init, use_bias=True,bias_initializer=const_init)(X)        
        X = Dense(64, activation="relu", kernel_initializer=truncatedn_init, use_bias=True,bias_initializer=const_init)(X)
        ...
        if dueling:
            state_value = Dense(1,kernel_initializer=truncatedn_init, bias_initializer=const_init, use_bias=True, activation = 'softmax')(X)
            state_value = Lambda(lambda s: K.expand_dims(s[:, 0], -1), output_shape=(action_space,))(state_value)

            action_advantage = Dense(action_space,kernel_initializer=truncatedn_init, bias_initializer=const_init, use_bias=True, activation = 'linear')(X)
