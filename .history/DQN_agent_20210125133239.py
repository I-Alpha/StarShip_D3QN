...
class DQN:
    ...
    def build_modelGPU(self, input_shape=(4, 100, 100,), action_space=6, dueling=True):
        ...
        X = Dense(128, activation="relu", kernel_initializer=hr)(X)        
        X = Dense(64, activation="relu", kernel_initializer=hr)(X)
        ...
        if dueling:
            state_value = Dense(1,kernel_initializer=hr)(X)
            state_value = Lambda(lambda s: K.expand_dims(s[:, 0], -1), output_shape=(action_space,))(state_value)

            action_advantage = Dense(action_space,kernel_initializer=hr)(X)
            action_advantage = Lambda(lambda a: a[:, :] - K.mean(a[:, :], keepdims=True), output_shape=(action_space,))(action_advantage)

            X = Add()([state_value, action_advantage])
        else:
            # Output Layer with # of actions: 2 nodes (left, right)
            X = Dense(action_space, activation="linear",kernel_initializer=hr, bias_initializer=const_init)(X)
