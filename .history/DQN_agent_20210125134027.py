...
class DQN:
    ...
    def build_modelGPU(self, input_shape=(4, 100, 100,), action_space=6, dueling=True):
        ...
        X = Conv2D(32, 8, strides=(4),padding="valid", kernel_initializer=truncatedn_init, bias_initializer=const_init, use_bias=True, activation="relu" , data_format="channels_first")(X)
        X = Conv2D(64, 4, strides=(2),padding="valid", kernel_initializer=truncatedn_init, bias_initializer=const_init, use_bias=True,activation="relu", data_format="channels_first")(X)
        X = Conv2D(128, 4, strides=(2),padding="same",kernel_initializer=truncatedn_init, bias_initializer=const_init,  use_bias=True,activation="relu", data_format="channels_first")(X)
        X = Flatten()(X)
        ...
        if dueling:
            ...
            X = Dense(action_space, activation="linear",kernel_initializer='he_uniform', bias_initializer=const_init)(X)
