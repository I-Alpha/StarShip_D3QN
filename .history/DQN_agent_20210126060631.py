...
class DQN:
    ...
    def saveModel(self, score="n.a"):
        print(self.model.name+"-" + str(DQN.currEpisode) +
              str(int(score)) + " saved! ")
        self.model.save("savedModels\\"+self.model.name +
                        "\\CNN-{}-{:.2f}.h5".format(DQN.currEpisode, score), overwrite=True)
    ...
    def build_modelGPU(self, input_shape=(4, 100, 100,), action_space=6, dueling=True):
        ...
        X = Conv2D(32, 8, strides=(4), padding="valid", activation="elu",
                   kernel_initializer=x_init, data_format="channels_first")(X)
        X = Conv2D(64, 4, strides=(2), padding="valid", activation="elu",
                   kernel_initializer=x_init,   data_format="channels_first")(X)
        X = Conv2D(128, 4, strides=(2), padding="valid", activation="elu",
                   kernel_initializer=x_init,   data_format="channels_first")(X)
        X = Flatten()(X)
        ...
        if dueling:
            ...
            X = Dense(action_space, activation="elu",
        ...
        model.compile(loss="mean_squared_error", optimizer=RMSprop(
            lr=self.learning_rate, epsilon=self.epsilon), metrics=["accuracy"])
    ...
    for e in range(lchk, episode):
        ...
        for i in range(max_steps):
            ...
            if done:
                ...
                print("episode: {}/{}, score: {}, average: {} , time-step-reached: {} ".format(
                    e, episode, score, average, i))
                # print("Max: ",i," Ep: ",e)
