    ...
    for e in range(lchk,episode):   
        ...
        for i in range(max_steps):
            ...
            if done:
                ...
                training_summary = tf.Summary(value=[tf.Summary.Value(tag="loss", simple_value=gl_loss),tf.Summary.Value(tag="average", simple_value=gl_average),
                tf.Summary.Value(tag="score", simple_value=gl_score)
                ,tf.Summary.Value(tag="max-step", simple_value=i),tf.Summary.Value(tag="dead obstacles", simple_value=env.deadObstacles)])
