...
tfv2.summary.trace_on(graph=True, profiler=True)
file_writer = tf.summary.FileWriter(log_dir, g)  
file_writer.add_graph(summary_ops_v2.graph(get_graph(), step=0))
gl_total_frames=0
gl_score =0
gl_loss=0
...
def train_dqn(episode,  graphics=True, ch=300,  lchk = 0 , model=None):    
    ...
    for e in range(lchk,episode):   
        ...
        with file_writer.as_default():
              tfv2.Summary. trace_on(name="model_trace", step=0, profiler_outdir=log_dir)
        score = 0
        for i in range(max_steps):
            ...
            if done:
                ...
                break          
                    ...
                    training_summary = tf.Summary(value=[
                    tf.Summary.Value(tag="loss", simple_value=gl_loss),
                    tf.Summary.Historgram(tag="loss_hist", simple_value=gl_loss),
                    tf.Summary.Value(tag="score", simple_value=gl_score),
                    tf.Summary.Historgram(tag="score_hist", simple_value=gl_score),
                    tf.Summary.Value(tag="max-step", simple_value=i),
                    tf.Summary.Historgram(tag="max-step_hist", simple_value=i),
                    tf.Summary.Value(tag="dead obstacles", simple_value=env.obstacleGenerator.deadObstacles),
                    tf.Summary.Historgram(tag="dead obstacles_hist", simple_value=env.obstacleGenerator.deadObstacles)
            ])
