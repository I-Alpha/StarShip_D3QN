        ...
        for i in range(max_steps):
            ...
            if done:
                ...
                break          
                  ...
                  training_summary = tf.Summary(value=[
                    tf.Summary.Value(tag="loss", simple_value=gl_loss),
                    tf2.Summary.histogram_summary(tag="loss_hist", simple_value=gl_loss),
                    tf.Summary.Value(tag="score", simple_value=gl_score),
                    tf.Summary.histogram_summary(tag="score_hist", simple_value=gl_score),
                    tf.Summary.Value(tag="max-step", simple_value=i),
                    tf.Summary.histogram_summary(tag="max-step_hist", simple_value=i),
                    tf.Summary.Value(tag="dead obstacles", simple_value=env.obstacleGenerator.deadObstacles),
                    tf.Summary.histogram_summary(tag="dead obstacles_hist", simple_value=env.obstacleGenerator.deadObstacles)
            ])
