        ...
        for i in range(max_steps):
            ...
            if done:
                ...
                break          
                  ...
                  training_summary = tf.Summary(value=[
                    tf.ummary.Value(tag="loss", simple_value=gl_loss),
                    tf2.summary.summary(tag="loss_hist", simple_value=gl_loss),
                    tf.summary.Value(tag="score", simple_value=gl_score),
                    tf.summary.summary(tag="score_hist", simple_value=gl_score),
                    tf.summary.Value(tag="max-step", simple_value=i),
                    tf.summary.summary(tag="max-step_hist", simple_value=i),
                    tf.summary.Value(tag="dead obstacles", simple_value=env.obstacleGenerator.deadObstacles),
                    tf.summary.summary(tag="dead obstacles_hist", simple_value=env.obstacleGenerator.deadObstacles)
            ])
